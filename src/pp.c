...
static struct token macro_token(struct pp *pp) {
        ...
        struct token *tok =
            VEC_INDEX(&macro->tokens, macro->token_idx++, struct token);
        return *tok;
}
...
static void skip_to_endif(struct pp *pp) {
        for (;;) {
                while (lex_next(CUR_LEXER(pp)).t != TOKEN_POUND)
                        ;
                struct token tok;
                if ((tok = lex_next(CUR_LEXER(pp))).t == TOKEN_ID &&
                    strrange(tok.v.id, "endif"))
                        return;
        }
}
...
static struct token pp_get(struct pp *pp) {
        ...
        if (tok.t == TOKEN_POUND && tok.sol) {
                ...
                if (strrange(direct.v.id, "include")) {
                        ...
                        struct src_file *file = src_file_open_include(
                            filename.v.str.buf, filename.v.str.len);

                        if (file == NULL) {
                                printf("cannot open file '%.*s'\n",
                                       (int)filename.v.str.len,
                                       filename.v.str.buf);
                                exit(EXIT_FAILURE);
                        }
                ...
                if (strrange(direct.v.id, "ifndef")) {
                        ...
                        if (sym.t != TOKEN_ID) {
                                printf("expected identifier referring to macro "
                                       "definition\n");
                                exit(EXIT_FAILURE);
                        }
