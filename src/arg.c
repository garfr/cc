...
static bool
parse_num(const char *arg, int64_t *num_out) {
    errno = 0;
    char *endptr;
...
static int
fill_flag(struct arg_flag *flag, const char *arg) {
...
static int
parse_short(const char *flag, const char *arg, struct arg_flag **flags) {
...
arg_parse(int argc,
    ...
    *err = NULL;
	...
	if (streq(argv[i], "-") || strlen(argv[i]) == 0) {
	    ...
	    int ret = parse_short(argv[i] + 1, argc - i != 1
				  ? argv[i + 1]
				  : NULL, flags);
	    if (ret == -1)
		return false;
